...
local textureSplash = 'CandySplashes';
...
function onCreatePost()
	...
	if string.match(songName,'trick-or-flee')  == 'trick-or-flee' then
		enableNewSystem = 2
		textureSplash = 'CandySplashes'
	end
	...
	if sickTrack ~= getProperty('sicks') then
		if sus == false then
			if enableNewSystem == 2 and getPropertyFromClass('ClientPrefs','noteSplashes') == true and not sus then
				if textureSplash == 'CandySplashes' then
					splashOffsetX = 90
					splashOffsetY = 100
					splashScaleX = 0.8
					splashScaleY = 0.8
...
function onUpdate()
	if textureSplash == 'CandySplashes' then
		splashVelocity = 30
	elseif textureSplash == 'CandySplashes' then
		splashVelocity = 34
	else
		splashVelocity = 24
	end
